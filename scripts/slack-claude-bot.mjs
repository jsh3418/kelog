...
const CLAUDE_PATH = process.env.CLAUDE_PATH || "/Users/jsh3418/.local/bin/claude";

function run(cmd, opts = {}) {
  const result = execSync(cmd, { cwd: REPO_PATH, encoding: "utf-8", stdio: "pipe", ...opts });
  return result.trim();
}
...
app.event("app_mention", async ({ event, say }) => {
  ...
  if (!task) {
    await say({ text: "작업 내용을 입력해주세요. 예: `@claude-bot 블로그에 /about 페이지 추가해줘`", thread_ts: event.ts });
    return;
  }

  if (isRunning) {
    await say({ text: "이전 작업이 진행 중입니다. 완료 후 다시 요청해주세요.", thread_ts: event.ts });
    return;
  }
  ...
  try {
    ...
    if (!status) {
      await say({ text: "변경사항이 없습니다. 작업이 필요하지 않거나 이미 완료된 상태입니다.", thread_ts: event.ts });
    ...
    await say({ text: `작업 중 에러가 발생했습니다:\n\`\`\`${errMsg.slice(0, 500)}\`\`\``, thread_ts: event.ts });
